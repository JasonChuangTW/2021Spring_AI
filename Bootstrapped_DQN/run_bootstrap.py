...
class ActionGetter:
    ...
    def pt_get_action(self, step_number, state, active_head=None, evaluation=False):
        ...
        if self.random_state.rand() < eps:
            ...
            state = torch.Tensor(state.astype(np.float64)/info['NORM_BY'])[None,:].to(info['DEVICE'])
...
def ptlearn(states, actions, rewards, next_states, terminal_flags, masks):
    states = torch.Tensor(states.astype(np.float64)/info['NORM_BY']).to(info['DEVICE'])
    next_states = torch.Tensor(next_states.astype(np.float64)/info['NORM_BY']).to(info['DEVICE'])
    rewards = torch.Tensor(rewards).to(info['DEVICE'])
    actions = torch.LongTensor(actions).to(info['DEVICE'])
    terminal_flags = torch.Tensor(terminal_flags.astype(np.int64)).to(info['DEVICE'])
    masks = torch.FloatTensor(masks.astype(np.int64)).to(info['DEVICE'])
...
if __name__ == '__main__':
    ...
    info = {
        ...
        "GAME":os.path.join('roms','boxing.bin'), # gym prefix
        "DEVICE":device, #cpu vs gpu set by argument
        "NAME":'FRANKbootstrap_fasteranneal_Boxing', # start files with name
